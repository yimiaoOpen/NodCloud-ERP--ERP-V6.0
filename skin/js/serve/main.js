layui.use('table', function() {
    ...
    table.on('tool(table_main)', function(obj){
        ...
        if(event == 'edit'){
            ...
            delect(data.id);//常规删除
        }else if(event == 'serveinfo'){
            serveinfo(data.id);//明细信息
        }
    });
    //监听批量操作
...
//详情
function detail(id){
    var html = '<div class="pop_box"><div class="layui-form layui-form-pane layui-row layui-col-space3"><div class="layui-col-md12"><div class="layui-form-item"><label class="layui-form-label">服务名称</label><div class="layui-input-block"><input type="text"id="name"placeholder="请输入服务名称"class="layui-input"></div></div></div><div class="layui-col-md12"><div class="layui-form-item"><label class="layui-form-label">服务编号</label><div class="layui-input-block"><input type="text"id="number"placeholder="请输入服务编号"class="layui-input"></div></div></div><div class="layui-col-md12"><div class="layui-form-item"><label class="layui-form-label">服务价格</label><div class="layui-input-block"><input type="text"id="price"placeholder="请输入服务价格"class="layui-input"></div></div></div><div class="layui-col-md12"><div class="layui-form-item"><label class="layui-form-label">备注信息</label><div class="layui-input-block"><input type="text"id="data"placeholder="请输入备注信息"class="layui-input"></div></div></div><more></more></div></div>';
        ...
        layer.ready(function() {
            layer.open({
                ...
                success: function(layero, index) {
                    ...
                    //获取信息
                    if (id > 0) {
                        ajax('POST','/index/serve/get_serve',{
                            "id": id
                        },function(resule){
                ...
                btn1: function(layero) {
                    ...
                    if (reg_test('empty',info['name'])) {
                        dump('服务名称不可为空!');
                    }else if(!reg_test('numerical',info['price'])){
                        dump('服务价格不正确!');
                    }else {
                        //提交信息
                        info['id']=id;
                        ajax('POST','/index/serve/set_serve',info,function(resule){
...
function delect(info){
    layui.use(['layer','table'], function() {
        layer.confirm('您确定要删除所选数据吗？', {
            ...
            //发送请求
            ajax('POST','/index/serve/del_serve',{
                "arr": arr
            },function(resule){
                if(resule.state=='success'){
                    search();
...
function exports(){
    ...
    jump_info('【 数据请求中 】',"/index/serve/export_serve?"+info,true);
}