...
function exports(){
    ...
    jump_info('【 数据请求中 】',"/index/sale/bill_export?"+info,true);
}
//操作对账单
        ...
        layer.ready(function() {
            layer.open({
                ...
                success: function(layero, index) {
                    ...
                    //获取信息
                    ajax('POST','/index/sale/bill_info',{
                        "id": data.id
                    },function(resule){
                ...
                btn1: function(layero) {
                    if(data.actual==data.money){
                        ...
                        layer.confirm('操作说明:<br/>1.核销状态将改为强制核销。<br/>2.强制核销等同于已核销状态。<br/>3.请再次确认是否执行该操作。', {
                            ...
                            //发送请求
                            ajax('POST','/index/sale/force_bill',{
                                "id": data.id
                            },function(resule){
                ...
                btn1: function(layero) {
                    ...
                    if(arr.length==0){
                        ...
                        layer.confirm('提交数据后不可批量逆操作,请再次确认是否操作?', {
                            ...
                            //发送请求
                            ajax('POST','/index/sale/set_bills',{
                                "arr": arr,
                                "account": account,
                                "money": money,
                                "data": data
