...
function pop_data(arr,type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var row={
            ...
            discount:type?'1':arr[i].discount,
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if(data[i].set_id !== '-1'){
            ...
            if(!reg_test('plus',data[i].nums) || data[i].nums=='0'){
                ...
                dump('第'+(i+1)+'行销货单价不正确!');
                return false;
            }else if(!reg_test('plus',data[i].discount) || (data[i].discount-0) == 0 || (data[i].discount-0) > 1){
                dump('第'+(i+1)+'行折扣不正确!');
                return false;
            }else{
                //计算行数据
                var total=cal((data[i].nums-0)*(data[i].price-0)*(data[i].discount-0));//计算行总价
