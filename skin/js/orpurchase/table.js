...
function pop_data(arr,type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        //处理仓库数据
        if(goodsinfo.warehouseinfo){
            row['warehouse'] = goodsinfo.warehouseinfo.name;
            row['warehouse_id'] = goodsinfo.warehouseinfo.id;
        }
        //兼容扩展字段
        if(arr[i].more){
        ...
        if(!reg_test('plus',data[i].nums)){
            ...
            if(data[i].nums !='0'){
                ...
                if(data[i].warehouse_id=='0'){
                    dump('第'+(i+1)+'行未选择所入仓库!');
                    return false;
                }else if((data[i].nums-0)>(data[i].surplusnums-0)){
                    dump('第'+(i+1)+'行本次数量不可大于剩余数量!');
                    return false;
...
//统计数据
function cal_total(){
    var allnums = cal($(grid_id).getCol('allnums', false, 'sum')); //总数量合计
    var surplusnums = cal($(grid_id).getCol('surplusnums', false, 'sum')); //剩余数量合计
    var nums = cal($(grid_id).getCol('nums', false, 'sum')); //本次数量合计
    var total = cal($(grid_id).getCol('total', false, 'sum')); //总价合计
    $(grid_id).footerData("set", {"allnums": "总数量:"+allnums,"surplusnums": "剩余总数量:"+surplusnums,"nums": "本次总数量:"+nums, "total": "总金额:"+total });//设置合计
