layui.use('table', function() {
    ...
    table.render({
        ...
        even: true,
        cols:  [formfield],
        url: '/index/repurchase/form_list',
        page: true,
        limit: 30,
...
function auditing(){
    layui.use(['layer','table'], function() {
        layer.confirm('您确定要[审核|反审核]所选数据吗？', {
            ...
            //发送请求
            ajax('POST','/index/repurchase/auditing',{"arr": arr},function(resule){
...
function delect(info){
    layui.use(['layer','table'], function() {
        layer.confirm('您确定要删除所选数据吗？', {
            ...
            //发送请求
            ajax('POST','/index/repurchase/del',{
                "arr": arr
            },function(resule){
                if(resule.state=='success'){
                    search();
...
function exports(type){
    ...
    jump_info('【 数据请求中 】',"/index/repurchase/exports?"+info+"&mode="+type,true);
}
//打印操作
function prints(id){
    layui.use('form', function(){
        layer.open({
          ...
          offset: '9%',
          area: ['650px', '330px'],
          content: '/index/repurchase/prints?id='+ id
        }); 
    });
}