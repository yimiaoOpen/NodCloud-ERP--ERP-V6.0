...
function info(id){
    layui.use('layer', function() {
        layer.open({
            ...
            area: ['99%', '98%'],
            shadeClose: true,
            content: '/index/otsale/info?id='+id,
            end:function(){
                search();
            }
        });
...
function auditing(){
    layui.use(['layer','table'], function() {
        layer.confirm('您确定要[审核|反审核]所选数据吗？', {
            ...
            //发送请求
            ajax('POST','/index/otsale/auditing',{"arr": arr},function(resule){
...
function exports(type){
    ...
    jump_info('【 数据请求中 】',"/index/otsale/exports?"+info+"&mode="+type,true);
}
//打印操作
function prints(id){
    layui.use('form', function(){
        layer.open({
          ...
          offset: '9%',
          area: ['650px', '330px'],
          content: '/index/otsale/prints?id='+ id
        }); 
    });
}