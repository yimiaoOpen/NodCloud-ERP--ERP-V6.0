layui.use('table', function() {
    ...
    table.on('tool(table_main)', function(obj){
        ...
        var event = obj.event;
        if(event == 'min_prints'){
            min_prints(data.id);//小票
        }else if(event == 'prints'){
            prints(data.id);//打印
...
function info(id){
    layui.use('layer', function() {
        layer.open({
            ...
            area: ['99%', '98%'],
            shadeClose: true,
            content: '/index/cashier/info?id='+id,
            end:function(){
                search();
            }
        });
...
function auditing(){
    layui.use(['layer','table'], function() {
        layer.confirm('您确定要[审核|反审核]所选数据吗？', {
            ...
            //发送请求
            ajax('POST','/index/cashier/auditing',{"arr": arr},function(resule){
...
function exports(type){
    ...
    jump_info('【 数据请求中 】',"/index/cashier/exports?"+info+"&mode="+type,true);
}
//打印操作
function prints(id){
    layui.use('form', function(){
        layer.open({
          ...
          offset: '9%',
          area: ['650px', '330px'],
          content: '/index/cashier/prints?id='+ id
        }); 
    });
}
//小票-打印操作
function min_prints(id){
    layui.use('form', function(){
        layer.open({
          type: 2,
          title: '小票打印',
          offset: '9%',
          area: ['650px', '330px'],
          content: '/index/cashier/min_prints?id='+ id
        }); 
    });
}