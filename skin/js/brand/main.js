...
//详情
function detail(id){
    var html = '<div class="pop_box"><div class="layui-form layui-form-pane layui-row layui-col-space3"><div class="layui-col-md12"><div class="layui-form-item"><label class="layui-form-label">品牌名称</label><div class="layui-input-block"><input type="text"id="name"placeholder="请输入品牌名称"class="layui-input"></div></div></div><div class="layui-col-md12"><div class="layui-form-item"><label class="layui-form-label">品牌编号</label><div class="layui-input-block"><input type="text"id="number"placeholder="请输入品牌编号"class="layui-input"></div></div></div><div class="layui-col-md12"><div class="layui-form-item"><label class="layui-form-label">备注信息</label><div class="layui-input-block"><input type="text"id="data"placeholder="请输入备注信息"class="layui-input"></div></div></div><more></more></div></div>';
        ...
        layer.ready(function() {
            layer.open({
                ...
                success: function(layero, index) {
                    ...
                    //获取信息
                    if (id > 0) {
                        ajax('POST','/index/brand/get_brand',{
                            "id": id
                        },function(resule){
                ...
                btn1: function(layero) {
                    ...
                    if (reg_test('empty',info['name'])) {
                        ...
                        //提交信息
                        info['id']=id;
                        ajax('POST','/index/brand/set_brand',info,function(resule){
...
function delect(info){
    layui.use(['layer','table'], function() {
        layer.confirm('您确定要删除所选数据吗？', {
            ...
            //发送请求
            ajax('POST','/index/brand/del_brand',{
                "arr": arr
            },function(resule){
                if(resule.state=='success'){
                    search();
