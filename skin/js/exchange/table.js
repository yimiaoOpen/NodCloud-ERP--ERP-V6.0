...
function pop_data(arr,type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var row={
            ...
            integral:type?((goodsinfo.retail-0)*(sys.integral_proportion-0)):arr[i].integral,
            allintegral:type?((goodsinfo.retail-0)*(sys.integral_proportion-0)):arr[i].allintegral,
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if(data[i].set_id !== '-1'){
            ...
            if(!reg_test('plus',data[i].nums) || data[i].nums=='0'){
                ...
                return false;
            }else if(!reg_test('plus',data[i].integral)){
                dump('第'+(i+1)+'行兑换积分不正确!');
                return false;
            }else{
                //计算行数据
                var allintegral=cal((data[i].nums-0)*(data[i].integral-0));//计算行总积分
                $(grid_id).jqGrid('setCell',data[i].id,'allintegral',allintegral);//设置行总积分
            }
        }
    }
    cal_total();//统计数据
...
function cal_total(){
    ...
    var allintegral = cal($(grid_id).getCol('allintegral', false, 'sum')); //总价合计
    $(grid_id).footerData("set", {"nums": "总数量:"+nums,"allintegral": "总积分:"+allintegral});//设置合计
    $('#total').val(allintegral);//赋值单据金额
}
